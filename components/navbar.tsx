    ...
    <header className={`${isScrolled ? "bg-white/90 shadow-sm" : "bg-white"} sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/70`}> 
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6">
        <div className="flex h-16 items-center justify-between">
          {/* Logo (keep as-is) */}
          <Link href="/" className="flex items-center gap-2 lg:mr-30" onClick={() => setMenuOpen(false)}>
            <span className="text-2xl bg-gradient-to-r from-fuchsia-500 to-amber-500 bg-clip-text text-transparent">BookMyBirthdayParty</span>
            <span className="hidden sm:inline text-xs text-gray-500">Celebrate Life!</span>
          </Link>
